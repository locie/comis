within FBM.Heating;
model Heating_Radiators_DHW_STS
 "Hydraulic heating with embedded emission, DHW (with STS), no TES for heating"
  // fixme: no solar system is implemeted so far (adapt documentation)
   ...
   extends FBM.Heating.Interfaces.Partial_HydraulicHeating(
    ...
    minSup=true,
    TSupMin=273.15 + 25,
        redeclare Buildings.Fluid.HeatExchangers.Radiators.RadiatorEN442_2 emission[nZones](
...
QHeaSys = -sum(emission.heatPortCon.Q_flow) - sum(emission.heatPortRad.Q_flow)+ QDHW;
  ...
  connect(TTank[posTTop].T, ctrl_Heating.TTankTop) annotation (Line(
    ...
    connect(emission.heatPortCon, heatPortCon) annotation (Line(
      points={{152,67.2},{152,70},{142,70},{142,96},{-178,96},{-178,20},{-200,20}},
      color={191,0,0},
      smooth=Smooth.None));
  connect(emission.heatPortRad, heatPortRad) annotation (Line(
      points={{158,67.2},{158,72},{148,72},{148,100},{-180,100},{-180,-20},{-200,
          -20}},
      color={191,0,0},
      smooth=Smooth.None));
  connect(prescribedHeatFlow.port, heatPortEmb)
    annotation (Line(points={{-200,74},{-200,74},{-200,60}}, color={191,0,0}));
  annotation (
