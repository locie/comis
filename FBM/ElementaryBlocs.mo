...
This is a model to collect the heat flows from <i>m</i> heatports to one single heatport.
  ...
  model PumpSupply
    ...
    extends Interfaces.PartialPumpCircuit(redeclare
        ...
        addPowerToMedium=addPowerToMedium),       final useBalancingValve=true,
  ...
  equation
    connect(flowRegulator.P, power) annotation (Line(
        points={{11,69},{40,69},{40,108}},
        color={0,0,127},
        smooth=Smooth.None));
         if booleanInput then
       connect(booleanToReal.y, flowRegulator.m_flow_in)
      annotation (Line(points={{-13,78},{0,78},{0,72}},       color={0,0,127}));
    connect(booleanToReal.u, u2) annotation (Line(points={{-36,78},{-40,78},{
      ...
      annotation (Line(points={{0,108},{0,108},{0,72}},       color={0,0,127}));
         end if;
    annotation (Documentation(info="<html><p>
  ...
  equation
    connect(u, flowRegulator.m_flow_in) annotation (Line(
        points={{0,108},{0,68},{0,68},{0,72}},
        color={0,0,127},
        smooth=Smooth.None));
    if measurePower then
     connect(flowRegulator.P, power) annotation (Line(
        points={{11,69},{40,69},{40,108}},
        color={0,0,127},
        smooth=Smooth.None));
...
First implementationn</li>
    ...
    //protected
      Buildings.Fluid.FixedResistances.PressureDrop balancingValve(
        ...
        m_flow_nominal=m_flow_nominal,
        allowFlowReversal=false,
        dp_nominal=m_flow_nominal^2/(KvReturn^2*1e5)) if
                                    useBalancingValve
...
If ports with different media are connected, the simulation is asserted due to the check of parameter.
    ...
    record BlockPara
      ...
      parameter Boolean includePipes=false
        ...
        annotation(Dialog(group = "Elementary block Settings"));
      parameter Boolean measureSupplyT=true
      ...
      parameter Modelica.SIunits.Length Pipelength=5
                                                    "pipe length of the supply OR return branch" annotation(Dialog(group = "Elementary block Settings"));
      parameter Modelica.SIunits.Pressure dp=0 "Pressure drop over a single pipe"
        annotation(Dialog(group = "Pipes",
                         enable = includePipes));
      parameter Modelica.SIunits.ThermalConductivity InsuHeatCondu=0.04 annotation(Dialog(group = "Elementary block Settings"));
      //----Fluid parameters
      ...
      parameter Modelica.SIunits.MassFlowRate m_flow_nominal
       ...
       annotation(Dialog(group = "Elementary block Settings"));
      parameter Modelica.SIunits.MassFlowRate m_flow_small(min=0) = 1E-4*abs(m_flow_nominal)
        "Small mass flow rate for regularization of zero flow" annotation(Dialog(group = "Elementary block Settings"));
      ...
      parameter Boolean measurePower=true
        ...
        annotation(Dialog(group = "Elementary block Settings"));
      parameter Boolean realInput = true annotation(Dialog(group = "Elementary block Settings"));
      parameter Boolean booleanInput = false annotation(Dialog(group = "Elementary block Settings"));

      parameter Real fraKSupply(min=0, max=1) = 0.7
        "Fraction Kv(port_3->port_2)/Kv(port_1->port_2)" annotation(Dialog(group = "Elementary block Settings"));
      parameter Real[2] lSupply(each min=0, each max=1) = {0.01, 0.01}
        "Valve leakage, l=Kv(y=0)/Kv(y=1)" annotation(Dialog(group = "Elementary block Settings"));

      parameter Boolean reverseAction = false
        "Set to true for throttling the water flow rate through a cooling coil controller" annotation(Dialog(group = "Elementary block Settings"));
       parameter Real k(min=0, unit="1") = 0.05 "Gain of controller";
      parameter Modelica.SIunits.Time Ti(min=Modelica.Constants.small)=300
        "Time constant of Integrator block" annotation (Dialog(group = "Elementary block Settings", enable=
              controllerType == Modelica.Blocks.Types.SimpleController.PI or
              controllerType == Modelica.Blocks.Types.SimpleController.PID));
      parameter Modelica.SIunits.Time Td(min=0)=0.1
        "Time constant of Derivative block" annotation (Dialog(group = "Elementary block Settings", enable=
              controllerType == Modelica.Blocks.Types.SimpleController.PD or
              controllerType == Modelica.Blocks.Types.SimpleController.PID));

      parameter Modelica.Blocks.Types.SimpleController controllerType=
             Modelica.Blocks.Types.SimpleController.PID "Type of controller" annotation(Dialog(group = "Elementary block Settings"));
