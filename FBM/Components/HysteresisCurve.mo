...
block HysteresisCurve
  "Transform Real to Boolean signal with Hysteresis around a signal"
...
initial equation
  pre(y) = pre_y_start;
equation
  assert((Signal + uHigh) > (Signal - uLow),"Hysteresis limits wrong (uHigh <= uLow)");
  y = not pre(y) and u > Signal + uHigh or pre(y) and u >= Signal - uLow;
