...
model EmbeddedPipe
  ...
  extends FBM.Components.BaseClasses.EmissionTwoPort;
  extends Buildings.Fluid.Interfaces.LumpedVolumeDeclarations;
  replaceable parameter FBM.Components.BaseClasses.RadiantSlabChar RadSlaCha
    constrainedby FBM.Components.BaseClasses.RadiantSlabChar
...
annotation(Dialog(tab="Flow resistance"));
  ...
  Modelica.SIunits.ThermalInsulance R_w_val= Buildings.Utilities.Math.Functions.spliceFunction(
    x=rey-(reyHi+reyLo)/2,
  ...
  Buildings.Fluid.MixingVolumes.MixingVolume[nDiscr] vol(each nPorts=2, each m_flow_nominal = m_flow_nominal, each V=m/nDiscr/rho_default,
  ...
  FBM.Components.ParallelFixedResistanceDpM res(
  ...
  Buildings.Fluid.Sensors.Temperature senTemIn(redeclare package Medium = Medium)
...
protected
  ...
  Buildings.Utilities.Math.Functions.spliceFunction(x=m_flowMin/nParCir/A_pipe*pipeDiaInt/mu_default-(reyHi+reyLo)/2,
  ...
  Real m_flowSpLimit = Buildings.Utilities.Math.Functions.smoothMax(m_flowSp, m_flow_nominal/A_floor/100, m_flow_nominal/A_floor/100)
...
equation
  ...
  if useSimplifiedRt then
    // Koschenz eq 4-60
    R_t = (Buildings.Utilities.Math.Functions.inverseXRegularized(2*m_flowSpLimit*cp_default*nDiscr, deltaXR) + R_w_val + R_r_val + R_x_val);
  else
    // Koschenz eq 4-59
    R_t = (Buildings.Utilities.Math.Functions.inverseXRegularized(m_flowSpLimit*cp_default*nDiscr*(1-exp(-1/((R_w_val+R_r_val+R_x_val+R_c)*m_flowSpLimit*cp_default*nDiscr))), deltaXR)-R_c);
  end if;
  ...
  connect(heatFlowSolid.port, heatPortEmb) annotation (Line(
   ...
   annotation (
    ...
    <p>
    <p><b> Component import from the IDEAS library and adapted for the Buildings library </b></p>  
